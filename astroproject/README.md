...
```
                    astrobasis 天枢 (纯机制层，Python 3.12 stdlib only)
                   ╱    │    ╲
    astroframe 筑穹   astrocrawl 摘星   astroweave 织霞
     (插件平台运行时)   (爬虫引擎)       (工作流编排)

pip 依赖:   astroframe → astrobasis,  astrocrawl → astrobasis,  astroweave → astrobasis
entry_point: astrocrawl → astroframe.plugins,  astroweave → astroframe.plugins
```
...
| **astroweave** | 织霞 | 织其华者，舞万丈霞 | 工作流编排引擎 — 可组合 DAG 工作流（规划中） |

## 组件包

| 包 | 星名 | 定位 |
|------|------|------|
| **astrobasis** | 天枢 | 纯机制层 — logfmt 日志、POSIX 原子写入、JSON 兼容层、共享 Protocol |
...
pip install --config-settings editable_mode=compat -e astrobasis/ -e astroframe/ -e astrocrawl/

# 或按需安装
pip install --config-settings editable_mode=compat -e astrobasis/ -e astrocrawl/
...
```
AstroProject/
├── astrobasis/astrobasis/       # 天枢 — 纯机制：_logging _atomic _json_compat _types
...
├── astroweave/astroweave/       # 织霞 — 工作流引擎（规划中）
├── docs/                      # ADR、开发者指南、领域文档
...
```
                    astrobasis (pure mechanism layer, Python 3.12 stdlib only)
                   ╱    │    ╲
          astroframe  astrocrawl  astroweave
         (platform)   (crawler)   (workflow)

pip dependency: astroframe → astrobasis,  astrocrawl → astrobasis,  astroweave → astrobasis
entry_point:    astrocrawl → astroframe.plugins,  astroweave → astroframe.plugins
```
...
| **astroweave** | 织霞 | 织其华者，舞万丈霞 | Workflow orchestration engine — composable DAG workflows (planning) |

## Component Package

| Package | Star | Role |
|---------|------|------|
| **astrobasis** | 天枢 | Pure mechanism — logfmt logging, POSIX atomic I/O, JSON compat, shared protocols |
...
```
AstroProject/
├── astrobasis/astrobasis/       # 天枢 — pure mechanism: _logging _atomic _json_compat _types
...
├── astroweave/astroweave/       # 织霞 — workflow engine (planning)

"""logfmt 日志系统测试 — _logfmt_escape / _format_logfmt_message / LogfmtFormatter / LogfmtLogger / setup_root_logger."""

from __future__ import annotations

import logging

from astrocrawl.utils.logging import (
    JsonLogFormatter,
    LogfmtFormatter,
    LogfmtLogger,
    _format_exc_oneline,
    _format_logfmt_message,
    _logfmt_escape,
    setup_root_logger,
)
...
class TestLogfmtFormatter:
    ...
    def test_basic_output(self):
        ...
        assert "logger=astrocrawl.test" in output
        assert "event=hello key=val" in output

    def test_no_event_field_for_structured(self):
        """LogfmtFormatter 不读取 _logfmt_event——它只依赖 getMessage()"""
        fmt = LogfmtFormatter(datefmt="%H:%M:%S")
        record = self._make_record(_logfmt_event="real_event")
        output = fmt.format(record)
        # getMessage() returns record.msg = "event=hello key=val"
        assert "event=hello key=val" in output
        # Does NOT use _logfmt_event
...
class TestJsonLogFormatter:
    ...
    def test_collision_detection(self):
        ...
        assert data["ts"] != "user_ts"  # 原始 ts 保留

    def test_error_collision_with_exception(self):
        """用户字段 error + 异常同时存在 → error=异常消息, field_error=用户值。"""
        fmt = JsonLogFormatter(datefmt="%H:%M:%S")
        try:
            raise ValueError("bad")
        except ValueError:
...
class TestLogfmtLogger:
    ...
    def test_info_captures(self):
        log = LogfmtLogger("astrocrawl.test_info_captures")
    ...
    def test_debug_disabled(self):
        log = LogfmtLogger("astrocrawl.test_debug_disabled")
        records, captor = self._capture(log)
    ...
    def test_info_disabled(self):
        log = LogfmtLogger("astrocrawl.test_info_disabled")
        records, captor = self._capture(log)
        log._logger.setLevel(logging.WARNING)
    ...
    def test_bind_independent(self):
        log = LogfmtLogger("astrocrawl.test_bind_independent")
        a = log.bind(worker=1)
        b = log.bind(worker=2)
    ...
    def test_exception_method(self):
        log = LogfmtLogger("astrocrawl.test_exception")
        records, captor = self._capture(log)
        try:
            raise ValueError("bad")
        except ValueError:
    ...
    def test_exc_info_keyword(self):
        log = LogfmtLogger("astrocrawl.test_exc_info")
        records, captor = self._capture(log)
    ...
    def test_name_property(self):
        log = LogfmtLogger("astrocrawl.test_name")
        assert log.name == "astrocrawl.test_name"

    def test_isEnabledFor(self):
        log = LogfmtLogger("astrocrawl.test_isenabled")
        log._logger.setLevel(logging.WARNING)
    ...
    def test_all_six_methods(self):
        log = LogfmtLogger("astrocrawl.test_all_six")
    ...
    def test_msg_includes_event_and_fields(self):
        log = LogfmtLogger("astrocrawl.test_msg_fields")
        records, captor = self._capture(log)
...
class TestSetupRootLogger:
    ...
    def test_backward_compatible_call(self):
        ...
        prev_level, prev_handlers = root.level, list(root.handlers)
        try:
            setup_root_logger(logging.DEBUG, "/tmp/test_astrocrawl.log")
            for h in root.handlers:
                if getattr(h, "_astrocrawl_handler", False):

...
class ContextPool:
    ...
    def __init__(
        ...
        self._log = logging.getLogger("astrocrawl.contextpool")
    ...
    async def init(self) -> None:
        ...
        for idx, result in enumerate(results):
            if isinstance(result, Exception):
                self._log.warning("event=slot_parallel_create_failed idx=%d error=%s", idx, result)
    ...
    async def scoped_path(self, idx: int, target: str) -> AsyncIterator[None]:
        ...
        try:
            ...
            try:
                ...
                self._log.warning("event=scoped_path_restore_failed idx=%d", idx, exc_info=True)

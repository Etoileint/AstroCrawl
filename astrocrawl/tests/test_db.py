...
class TestTransactionCancellationSafety:
    """_transaction() 必须在 CancelledError 后保持连接可用。

    aiosqlite 后台线程与 asyncio 取消机制解耦——CancelledError 可中断
    await future 但后台线程仍会执行已入队的 SQL。若 BEGIN IMMEDIATE 在
    CancelledError 到达时未受保护，连接遗留孤儿事务，后续所有写入全部失败。
    """

    async def test_rapid_cancel_does_not_poison_connection(self, state):
        """高频取消事务后连接必须仍然可用。

        200 次极短超时的 push_to_queue_single——足够多次命中 CancelledError
        落在 BEGIN IMMEDIATE 边界上。只要一次未保护 = 连接永久毒化 = 最终
        push 失败。
        """

        async def _cancellable_push(i: int):
            try:
                await asyncio.wait_for(
                    state.push_to_queue_single(f"https://cancel-{i}.com/1", 1),
                    timeout=0.000001,
                )
            except (asyncio.TimeoutError, asyncio.CancelledError):
                pass

        tasks = [asyncio.create_task(_cancellable_push(i)) for i in range(200)]
        await asyncio.gather(*tasks)

        result = await state.push_to_queue_single("https://final-after-cancel.com/1", 1)
        assert result.name == "ENQUEUED", f"Connection poisoned after 200 cancel attempts: {result}"

    async def test_cancel_during_long_transaction_recovered(self, state):
        """事务内容被取消后连接必须可用（取消发生在 COMMIT 前）。"""

        async def _long_transaction():
            async with state._transaction():
                await state._db.execute(
                    "INSERT OR IGNORE INTO queue(url, depth, added_time, domain) VALUES(?,?,?,?)",
                    ("https://long-txn.com/1", 1, time.time(), "long-txn.com"),
                )
                await asyncio.sleep(0.05)  # 给取消窗口
                pytest.fail("Should have been cancelled before commit")

        task = asyncio.create_task(_long_transaction())
        await asyncio.sleep(0.01)  # 事务已开始
        task.cancel()
        with pytest.raises(asyncio.CancelledError):
            await task

        # 回滚后连接必须干净
        result = await state.push_to_queue_single("https://after-rollback.com/1", 1)
        assert result.name == "ENQUEUED", f"Connection not clean after rollback: {result}"
        assert await state.queue_size() == 1

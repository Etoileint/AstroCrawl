...
from ._fakes import FakeBrowserPool, FakeWriter  # noqa: E402
...
def fake_prefs():
    ...
    from ._fakes_gui import FakePreferences
...
def fake_crawl_session():
    ...
    from ._fakes_gui import FakeCrawlSession

    return FakeCrawlSession()


@pytest.fixture(autouse=True)
def _disable_plugin_loading(request, monkeypatch):
    """ADR-0012: 在所有测试中禁用插件加载（ASTROCRAWL_PLUGINS=0）。

    标记 @pytest.mark.plugin_migration 的测试自动获得 ASTROCRAWL_PLUGINS=1。
    保护 3,896+ 个现有测试不受插件发现副作用干扰。
    """
    if request.node.get_closest_marker("plugin_migration"):
        monkeypatch.setenv("ASTROCRAWL_PLUGINS", "1")
    else:
        monkeypatch.setenv("ASTROCRAWL_PLUGINS", "0")

...
class CrawlState:
    ...
    async def _transaction(self):
        ...
        asyncio.shield() 保护事务边界不被 CancelledError 中断——已入队 aiosqlite
        后台线程的 BEGIN/COMMIT/ROLLBACK 必须执行完毕，否则连接遗留孤儿事务导致
        全局毒化（所有后续 _transaction() 调用均失败于 "cannot start a transaction
        within a transaction"）。
        """
        async with self._write_lock:
            try:
                await asyncio.shield(self._db.execute("BEGIN IMMEDIATE"))
                yield
                await asyncio.shield(self._db.commit())
            except _Rollback:
                await asyncio.shield(self._db.execute("ROLLBACK"))
                raise

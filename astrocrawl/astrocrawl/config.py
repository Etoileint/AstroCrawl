...
_log = logging.getLogger("astrocrawl.config")
...
DEFAULT_USER_AGENT = f"Mozilla/5.0 (compatible; AstroCrawl/{__version__}; +https://github.com/Etoileint/AstroProject)"
...
class CrawlerConfig:
    ...
    def from_dict(cls, data: dict) -> "CrawlerConfig":
        ...
        if unknown:
            _log.warning("event=config_unknown_fields fields=%s", ", ".join(sorted(unknown)))
    ...
    def from_env(cls, base: "CrawlerConfig | None" = None) -> "CrawlerConfig":
        ...
        for env_key, (fname, cast) in env_map.items():
            ...
            if val is not None:
                try:
                    ...
                    _log.warning("event=env_var_invalid env_key=%s value=%s", env_key, val)
        ...
        if rules_max_generic_env:
            try:
                ...
                _log.warning(
                    "event=env_var_invalid env_key=ASTROCRAWL_RULES_MAX_GENERIC value=%s", rules_max_generic_env
                )
